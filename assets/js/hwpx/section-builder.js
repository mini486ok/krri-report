...
function labelRange(text) {
  return P(PARA.LABEL_TITLE, CHAR.LABEL_TEXT, text, { preset: 'label' });
}
...
function kindLabel(kind, idx) {
  const names = { basic: '기본사업', consign: '수탁사업', etc: '기타' };
  return `(${idx}) ${names[kind] ?? kind}`;
}
...
export function buildBodySubList(round, submissions, side, { orgName = '[철도AI융합연구실]' } = {}) {
  ...
  kinds.forEach((kind, i) => {
    const kindsCats = categories.filter(c => c.kind === kind);
    // 해당 kind에 카테고리가 전혀 없으면 대분류 라인도 스킵
    if (kindsCats.length === 0) return;

    // 대분류 라인
    if (kind === 'etc') {
      parts.push(P(PARA.ETC_KIND, CHAR.ETC_KIND_TEXT, kindLabel(kind, i + 1)));
    } else {
      parts.push(P(PARA.ORG_LINE, CHAR.ORG_AND_KIND, kindLabel(kind, i + 1)));
    }
    ...
    for (const cat of kindsCats) {
      ...
      if (kind !== 'etc') {
        parts.push(Pmulti(PARA.PROJECT_LINE, [
          [CHAR.ORG_AND_KIND, ' - '],
          [CHAR.PROJECT_TITLE, `${cat.title}(${cat.owner})`],
        ]));
      }
      for (const it of items) {
        parts.push(P(PARA.BULLET, CHAR.BULLET_TEXT, formatItem(it), { preset: 'bullet' }));
      }
    }
    // 카테고리 섹션 끝에 빈 줄
